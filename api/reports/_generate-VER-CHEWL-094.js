...
async function generateKpiReport({ reportType, startDate, endDate, decoded, res }) {
  ...
  if (reportType === 'kpi_report' && startDate && endDate) {
    ...
    // Determine if a date is a "holiday day" project-wide:
    // if ANY staff has Day Status = Holiday on that date, treat as project holiday.
    ...
    const coordCell = (proj, date) => {
      ...
      // Sum of Sheet KPI for the day (used as project-day output indicator)
      let total = 0;
      for (const st of Object.values(proj.staff)) {
    ...
    // Determine which KPI types each project actually has records for.
    // Skip sections that have zero records \u2014 e.g. OT Sky Palace has only
    // Model + Sheet KPI in the source data, no Parameter KPI.
    ...
    const projectBlocks = sortedProjects.map((proj, idx) => {
      ...
      const pivotTable = (type) => {
        // Skip section if no records exist for this KPI type in this project.
        if (!availTypes.has(type)) return '';
        const sec = sectionColors[type];
        // Only include staff who have at least one record of THIS KPI type.
    ...
    // Plain-text markdown fallback (used if PDF rendering falls back)
    const mdLines = [
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Standalone KPI-family reports (no project required)
    ...
    // Portfolio / project reports - fetch from projects_data.json
