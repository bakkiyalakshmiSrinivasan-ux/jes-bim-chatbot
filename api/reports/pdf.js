...
function renderHTML(markdown, { title, subtitle, kind, reportLabel }) {
  ...
  // Paragraph/line breaks (run AFTER tables)
  html = html
    .replace(/\n\n/g, '</p><p>')
    .replace(/\n/g, '<br>');
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .page { padding: 28px 40px 90px 40px; }
  .title {
    font-size: 22pt;
    font-weight: 800;
    color: ${theme.primary};
  ...
  .subtitle { font-size: 10pt; color: #334155; margin: 0 0 18px 0; }
  h1.section-title, h2 {
    color: ${theme.primary};
    font-size: 13pt;
    font-weight: 700;
  ...
  table.tp-table tr:nth-child(even) td { background: ${theme.tableAltRow}; }
